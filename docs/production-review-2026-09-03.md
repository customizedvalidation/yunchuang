...
| 2 | grafana 弱口令 | **已彻底修复（原记录有误，此处更正）**：`docker/docker-compose.yml` 的 `GF_SECURITY_ADMIN_PASSWORD=${GRAFANA_ADMIN_PASSWORD:-admin}` → `${GRAFANA_ADMIN_PASSWORD:?GRAFANA_ADMIN_PASSWORD is required, see .env.example}`，与另外两份 compose（根 `docker-compose.yml:72`、`metaclouds-backend/docker-compose.yml:160`）的 fail-fast 写法一致。<br>**原「保留默认以避免破坏 `tests/docker_multi_instance_test.go` 联调栈」的理由经核查不成立**：该测试只请求 `http://localhost:8000`（后端 API，凭据 `admin/admin123`），全文零 grafana 引用，且服务不可用时 `t.Skip` 优雅跳过，并不依赖 grafana 口令。变量早在根 `.env.example:27` 登记。 | 三份 compose 均无 `:-admin` 类弱默认；YAML 解析通过（7 services） |
